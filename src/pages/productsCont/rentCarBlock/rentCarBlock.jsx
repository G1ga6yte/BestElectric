...
import carImg1 from "../carOrderBlock/images/image 16.png";
import img1 from "./images/rentImg.png";
import {RentImages} from "./images/images";
import {use} from "i18next";

function RentCarBlock (){
  const {t, i18n} = useTranslation();
  const [sortBlockActive, setSortBlockActive] = useState(false)
  const [sortBlockVisible, setSortBlockVisible] = useState(false)
  const [itemsShowCount, setItemsShowCount] = useState(6)
  ...
  return(
     <div className="productsMainBlock RentCarBlock">
       <div className="G-CContainer">
         <p className="header G-header-100 G-black no-select"><TextInView text={t("products.header6")}/></p>
         <p className="prg G-16-300-Inter G-black no-select"><TextInView text={t("products.underHeader4")}/></p>
       </div>
       ...
       <div style={{backgroundImage: `url("${img1}")`}} className="rentCarBlock G-flex-ACenter-JCenter G-flex-Column no-select">
         ...
         <button onClick={()=>setCalculateBlock(true)} className="calculateBtn G-greenBtn G-white G-20-400-Nexa"><TextInView text={t("products.btn3")}/></button>
       </div>
